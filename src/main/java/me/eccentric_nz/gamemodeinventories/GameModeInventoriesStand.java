        ...
        if (plugin.getConfig().getBoolean("track_creative_place.enabled")) {
            ...
            try (Connection connection = plugin.getDatabaseConnection();
                    PreparedStatement statement = connection
                            .prepareStatement("SELECT uuid FROM " + plugin.getPrefix() + "stands");
                    ResultSet rs = statement.executeQuery();)
            {
                ...
                try (PreparedStatement ps = connection
                        .prepareStatement("DELETE FROM " + plugin.getPrefix() + "stands");)
                {
    ...
    public void saveStands() {
        ...
        try (Connection connection = plugin.getDatabaseConnection();
                PreparedStatement ps = connection
                        .prepareStatement("INSERT INTO " + plugin.getPrefix() + "stands (uuid) VALUES (?)");)
        {
